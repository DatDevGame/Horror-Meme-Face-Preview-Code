...
namespace FancyScrollView
{
    ...
    public abstract class FancyScrollView<TItemData, TContext> : MonoBehaviour where TContext : class, new()
    {
        ...
        void UpdatePosition(float position, bool forceRefresh)
        {
            ...
            SetLastSiblingCell(position);

            UpdateCells(firstPosition, firstIndex, forceRefresh);

        }

        private void SetLastSiblingCell(float position)
        {
            int middleIndex = Mathf.CeilToInt(position);

            middleIndex = middleIndex < 0 ? 0 : middleIndex < pool.Count ? middleIndex : pool.Count - 1;

            var cell = pool[middleIndex];

            if(cell.HasGetData)
                pool[middleIndex]?.transform.SetAsLastSibling();
        }
        ...
        void ResizePool(float firstPosition)
        {
            ...
            //var addCount = GameManager.Instance.Resources.DefaultInterestSettings.InterestItem.CustomMissionDatas;
        ...
        void UpdateCells(float firstPosition, int firstIndex, bool forceRefresh)
        {
            for (var i = 0; i < pool.Count; i++)
            {
                ...
                if (index < 0 || index >= ItemsSource.Count || position > 0.8f || position < 0.2f)
                {
                    cell.SetVisible(false);
                    continue;
                }
